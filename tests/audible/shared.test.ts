...
describe('When building urls', () => {
    it('returned the correct url', () => {
        ...
        expect(
            helper.buildUrl(asin, baseDomain, baseUrl, params)
        ).toEqual('https://api.audible.com/1.0/catalog/products/B08G9PRS1K/?response_groups=contributors,product_desc,product_extended_attrs,product_attrs,media')
    })
    ...
    it('returned the correct url', () => {
        ...
        baseUrl = 'pd'
        expect(
            helper.buildUrl(asin, baseDomain, baseUrl)
        ).toEqual('https://www.audible.com/pd/B08G9PRS1K/')
    })
    ...
    it('returned the correct url', () => {
        ...
        expect(
            helper.buildUrl(asin, baseDomain, baseUrl, params)
        ).toEqual('https://api.audible.com/1.0/content/B08G9PRS1K/metadata?response_groups=chapter_info')
    })
})
...
describe('When checking asins', () => {
    it('returned true for a good asin', () => {
        expect(
            helper.checkAsinValidity(asin)
        ).toBeTruthy()
    })

    it('returned false for a long asin', () => {
        expect(
            helper.checkAsinValidity('B08G9PRS1K' + '1')
        ).toBeFalsy()
    })

    it('returned false for an invalid asin', () => {
        expect(
            helper.checkAsinValidity('B08G9PRS1+')
        ).toBeFalsy()
    })
})
